...
const API_HOST = process.env.ML_API_HOST;
...
export const parseEndpoint = (endpoint, params) => {
  ...
  const url = endpoint.indexOf('http') === 0 ? endpoint : `${API_HOST}/${_.get(store.getState(), 'auth.user.username', {})}/${strippedEndpoint}`;
  ...
  request(
    ...
    return fetch(parsedEndpoint, parsedSettings)
      ...
      .catch((e) => {
        ...
        console.log(parsedSettings);
        console.log(e);
        if (e.status === 401 && autoLogout) {   //Unauthorized
